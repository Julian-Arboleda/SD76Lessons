...
namespace _01_Types_and_Varibles
{
    [TestClass]
    public class ReferenceTypeExamples
    {
        private object listOffStrings;

        [TestMethod]
        public void Strings()
        {
            string firstName = "Julian ";
            string lastName = "Arboleda";

            string concatenated = firstName + " " + lastName;

            string interpolated = $"{firstName} {lastName}";

            string composited = string.Format("{0} {1}", firstName, lastName); //,middleName);
            // this is more powerful that interpolated because value can also be variables and it carries out it in order from 0, ie: 0=4 456
